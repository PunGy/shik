    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ParseError::InvalidNumber { lexeme, line, column } => {
                write!(f, "Invalid number '{}' at line {}, column {}", lexeme, line, column)
            }
            ParseError::ReservedIdentifier { lexeme, line, column } => {
                write!(f, "Reserved identifier '{}' at line {}, column {}", lexeme, line, column)
            }
            ...
            ParseError::UnterminatedInterpolationString { line, column } => {
                write!(f, "Unterminated interpolation string at line {}, column {}", line, column)
            }
            ParseError::UnexpectedToken { token, expected } => {
                write!(f, "Unexpected token '{}' at line {}, column {}. Expected: {}", 
                    token.lexeme, token.line, token.column, expected)
            }
            ParseError::UnexpectedEndOfInput { expected } => {
                write!(f, "Unexpected end of input. Expected: {}", expected)
            }
            ParseError::InvalidPattern { message, line, column } => {
                write!(f, "Invalid pattern: {} at line {}, column {}", message, line, column)
            }
        }
    }
}
